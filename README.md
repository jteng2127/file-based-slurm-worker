...
Arguments:
  ...
  MAX_IDLE                      Seconds to wait with no tasks before exiting (default: -1, run forever)
  ACCEPT_TASK_TIME_SECOND_LIMIT Limit in seconds after which the worker stops accepting new tasks (default: -1, no limit)
  LOG_SYSTEM_METRICS            1 to log system metrics before each task, 0 otherwise (default: 0)
  MAX_CONSECUTIVE_FAILS         Max consecutive task failures before worker auto-terminates (default: -1)
```
